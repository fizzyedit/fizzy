...
pub fn isModifier(k: Key) bool {
    return switch (k) {
        .left_shift, .right_shift, .left_control, .right_control, .left_alt, .right_alt, .left_command, .right_command => true,
        else => false,
    };
}
...
const spellings = [_]Spelling{
    ...
    // Named keys.
                        .{ .text = "left", .key = .left },
    .{ .text = "right", .key = .right },                 .{ .text = "up", .key = .up },
    .{ .text = "down", .key = .down },                   .{ .text = "pageup", .key = .page_up },
    .{ .text = "pagedown", .key = .page_down },          .{ .text = "page_up", .key = .page_up },
    .{ .text = "page_down", .key = .page_down },         .{ .text = "home", .key = .home },
    .{ .text = "end", .key = .end },                     .{ .text = "tab", .key = .tab },
    .{ .text = "enter", .key = .enter },                 .{ .text = "return", .key = .enter },
    .{ .text = "escape", .key = .escape },               .{ .text = "esc", .key = .escape },
    .{ .text = "space", .key = .space },                 .{ .text = "backspace", .key = .backspace },
    .{ .text = "delete", .key = .delete },               .{ .text = "del", .key = .delete },
    .{ .text = "insert", .key = .insert },               .{ .text = "capslock", .key = .caps_lock },
    .{ .text = "numlock", .key = .num_lock },            .{ .text = "scrolllock", .key = .scroll_lock },
    .{ .text = "pausebreak", .key = .pause },            .{ .text = "pause", .key = .pause },
    .{ .text = "printscreen", .key = .print },           .{ .text = "menu", .key = .menu },
    ...
    .{ .text = "numpad_multiply", .key = .kp_multiply }, .{ .text = "numpad_add", .key = .kp_add },
    .{ .text = "numpad_subtract", .key = .kp_subtract }, .{ .text = "numpad_divide", .key = .kp_divide },
    .{ .text = "numpad_decimal", .key = .kp_decimal },   .{ .text = "numpad_enter", .key = .kp_enter },
