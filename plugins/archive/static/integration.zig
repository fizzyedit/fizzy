//! Archive plugin — fizzy-internal static-embed + bundled-dylib module graph.
...
pub const id = "archive";
pub const installDylib = helpers.installDylib;

const module_path = "plugins/archive/plugin.zig";
const zon_path = "plugins/archive/plugin.zig.zon";
...
/// Static `@import("archive")` module for exe / web / tests.
...
/// Native dynamic library bundled beside the app (`archive.dylib` / `.dll` / `.so`).
pub fn addDylib(
    b: *std.Build,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
