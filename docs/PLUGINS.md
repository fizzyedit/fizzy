...
`fizzy.plugin.install` (which renames Zig's `libplugin.dylib` output to the `plugin.<ext>`
...
Then `zig build install --prefix <plugins>/<id>` lands `plugin.<ext>` where the host
expects it, e.g. `~/.config/fizzy/plugins/<id>/plugin.dylib` (macOS:
`~/Library/Application Support/fizzy/plugins/<id>/plugin.dylib`).
...
4. `zig build install --prefix <plugins>/<id>/` so the host finds `plugin.<ext>`.
