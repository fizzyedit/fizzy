...
// dvui types that cross the boundary (`dvui.Vertex`/`Texture`/`Rect.Physical`/`Id`, …) are no
// longer hashed as a separate list: `abi_fingerprint` is now a *shape* hash (see below), and the
// shape walk of `sdk_boundary_types`/`entry_symbol_types` already reaches every dvui type that
// actually crosses the boundary — by value or as a render-bridge fn-pointer param — one level
// down. dvui types the SDK only passes by pointer (e.g. `*dvui.Window`) don't affect ABI layout
// and are correctly not walked. A dvui update that restructures a boundary-reachable dvui type
// therefore still moves the fingerprint (and trips the `sdk_version` guard); a cosmetic dvui
// update that doesn't touch the boundary shape leaves it unchanged, so plugins keep loading.
