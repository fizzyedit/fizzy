...
pub const SyntaxHighlight = struct {
    ...
    /// Longest `highlights` entry whose name is a prefix of `capture_name`.
    pub fn optsForCapture(capture_name: []const u8, highlights: []const SyntaxHighlight) Match {
        var best: Match = .{};
        for (0..highlights.len) |i| {
            const sh = highlights[highlights.len - i - 1];
            if (std.mem.startsWith(u8, capture_name, sh.name) and sh.name.len > best.specificity) {
                best = .{ .opts = sh.opts, .specificity = @intCast(sh.name.len) };
            }
        }
        return best;
    }
...
/// Tree-sitter 0.27+ leaves `#match?` / `#eq?` / … to the host. Without this,
/// every `(identifier)` pattern matches every identifier regardless of predicates.
const QueryPredicates = if (dvui.useTreeSitter) struct {
    const Arg = union(enum) {
        capture: u32,
        string: []const u8,
    };

    fn captureTextInMatch(match: dvui.c.TSQueryMatch, capture_id: u32, text: []const u8) ?[]const u8 {
        var i: u32 = 0;
        while (i < match.capture_count) : (i += 1) {
            const cap = match.captures[i];
            if (cap.index == capture_id) {
                const start = dvui.c.ts_node_start_byte(cap.node);
                const end = dvui.c.ts_node_end_byte(cap.node);
    ...
    fn queryStringValue(query: *const dvui.c.TSQuery, id: u32) []const u8 {
        var len: u32 = undefined;
        const ptr = dvui.c.ts_query_string_value_for_id(query, id, &len);
        return ptr[0..len];
    }
    ...
    fn isMatchRegex(text: []const u8, pattern: []const u8) bool {
        ...
        if (std.mem.eql(u8, pattern, "^[A-Z][A-Z_0-9]+$")) {
            if (text.len == 0) return false;
            if (text[0] < 'A' or text[0] > 'Z') return false;
            for (text[1..]) |ch| {
                if ((ch >= 'A' and ch <= 'Z') or (ch >= '0' and ch <= '9') or ch == '_') continue;
                return false;
            }
            return true;
        }
    ...
    fn evalPredicate(
        name: []const u8,
        args: []const Arg,
        match: dvui.c.TSQueryMatch,
        text: []const u8,
    ) bool {
        if (std.mem.eql(u8, name, "#set!")) return true;

        if (std.mem.eql(u8, name, "#match?") or std.mem.eql(u8, name, "#lua-match?")) {
            if (args.len < 2) return true;
            const cap_text = switch (args[0]) {
                .capture => |id| captureTextInMatch(match, id, text) orelse return false,
                else => return false,
            };
            ...
            return isMatchRegex(cap_text, pattern);
        }
        ...
        if (std.mem.eql(u8, name, "#any-of?")) {
            ...
            for (args[1..]) |arg| {
                switch (arg) {
                    .string => |s| if (std.mem.eql(u8, cap_text, s)) return true,
                    else => {},
                }
            }
            return false;
    ...
    pub fn patternMatches(
        query: *const dvui.c.TSQuery,
        pattern_index: u16,
        match: dvui.c.TSQueryMatch,
        text: []const u8,
    ) bool {
        var step_count: u32 = 0;
        const steps = dvui.c.ts_query_predicates_for_pattern(query, pattern_index, &step_count);
        if (step_count == 0) return true;

        var i: u32 = 0;
        while (i < step_count) {
            const first = steps[i];
            if (first.type != dvui.c.TSQueryPredicateStepTypeString) {
                i += 1;
                continue;
            }
            const pred_name = queryStringValue(query, first.value_id);
            i += 1;

            var args: [16]Arg = undefined;
            var arg_count: usize = 0;
            while (i < step_count and steps[i].type != dvui.c.TSQueryPredicateStepTypeDone) {
                const step = steps[i];
                i += 1;
                if (arg_count >= args.len) break;
                switch (step.type) {
                    dvui.c.TSQueryPredicateStepTypeCapture => {
                        args[arg_count] = .{ .capture = step.value_id };
                        arg_count += 1;
                    },
                    dvui.c.TSQueryPredicateStepTypeString => {
                        args[arg_count] = .{ .string = queryStringValue(query, step.value_id) };
                        arg_count += 1;
                    },
                    else => {},
                }
            }
            if (i < step_count and steps[i].type == dvui.c.TSQueryPredicateStepTypeDone) i += 1;

            if (!evalPredicate(pred_name, args[0..arg_count], match, text)) return false;
        }
        return true;
    }
} else struct {
    pub fn patternMatches(_: *const dvui.c.TSQuery, _: u16, _: dvui.c.TSQueryMatch, _: []const u8) bool {
        return true;
    }
};
...
pub const InitOptions = struct {
    ...
    /// When false, skip the themed focus ring around the widget border.
    show_focus_border: bool = true,
...
pub fn init(self: *TextEntryWidget, src: std.builtin.SourceLocation, init_opts: InitOptions, opts: Options) void {
    ...
    if (self.data().options.backgroundGet() or self.data().options.borderGet().nonZero()) {
    ...
    self.scroll.init(@src(), scroll_init_opts, self.data().options.strip().override(.{
        .role = .none,
        .expand = .both,
        .background = false,
        .border = Rect{},
    ...
    if (dvui.useTreeSitter) {
        if (self.init_opts.tree_sitter) |ts| {
            ...
            const parser = dvui.dataGetPtr(null, self.data().id, "parser", TreeSitterParser) orelse blk: {
                ...
                if (query == null) {
                    dvui.log.err("TextEntryWidget tree-sitter query failed at offset {d}: {any}", .{ errorOffset, errorType });
                    dvui.c.ts_tree_delete(tree);
                    dvui.c.ts_parser_delete(p);
                    break :blk null;
                }

                const parser_state: TreeSitterParser = .{ .parser = p.?, .tree = tree.?, .query = query.? };
                dvui.dataSet(null, self.data().id, "parser", parser_state);
            ...
            const range_start: usize = if (self.textLayout.cache_layout_bytes) |clb| @min(self.len, clb.start) else 0;
            const range_end: usize = if (self.textLayout.cache_layout_bytes) |clb| @min(self.len, clb.end) else self.len;
            const range_len = range_end - range_start;

            const default_opts = self.data().options.strip();

            if (range_start > 0) {
                self.textLayout.addText(self.text[0..range_start], default_opts);
            }
            ...
            if (range_len > 0) {
                ...
                var spans: std.ArrayListUnmanaged(CaptureSpan) = .empty;
                defer spans.deinit(lifo);

                var match: dvui.c.TSQueryMatch = undefined;
                var capture_idx: u32 = undefined;
                while (dvui.c.ts_query_cursor_next_capture(qc, &match, &capture_idx)) {
                    if (!QueryPredicates.patternMatches(ts_parser.query, match.pattern_index, match, self.text)) continue;

                    const cap = match.captures[capture_idx];
                    const span_start = dvui.c.ts_node_start_byte(cap.node);
                    const span_end = dvui.c.ts_node_end_byte(cap.node);
                    if (span_end <= range_start or span_start >= range_end) continue;

                    var cap_len: u32 = undefined;
                    const cap_name = dvui.c.ts_query_capture_name_for_id(ts_parser.query, cap.index, &cap_len);
                    const highlight = SyntaxHighlight.optsForCapture(cap_name[0..cap_len], ts.highlights);
                    if (highlight.specificity == 0) continue;

                    if (ts.log_captures) {
                        dvui.log.debug("ts capture @{s} : {s}", .{ cap_name[0..cap_len], self.text[span_start..span_end] });
                    }
                    ...
                    spans.append(lifo, .{
                        ...
                        .specificity = highlight.specificity,
                    }) catch {
                        dvui.log.err("tree-sitter highlight span alloc failed", .{});
                        break;
                    };
                }

                const best_spec = lifo.alloc(u16, range_len) catch {
                    dvui.log.err("tree-sitter highlight buffer alloc failed", .{});
                    self.textLayout.addText(self.text[range_start..range_end], default_opts);
                    if (range_end < self.len) self.textLayout.addText(self.text[range_end..self.len], default_opts);
                    self.textLayout.addTextDone(default_opts);
                    self.drawAfterText();
                    return;
                };
                const best_span_len = lifo.alloc(u16, range_len) catch {
                    dvui.log.err("tree-sitter highlight buffer alloc failed", .{});
                    self.textLayout.addText(self.text[range_start..range_end], default_opts);
                    if (range_end < self.len) self.textLayout.addText(self.text[range_end..self.len], default_opts);
                    self.textLayout.addTextDone(default_opts);
                ...
                const best_cap = lifo.alloc(u32, range_len) catch {
                    dvui.log.err("tree-sitter highlight buffer alloc failed", .{});
                    self.textLayout.addText(self.text[range_start..range_end], default_opts);
                    if (range_end < self.len) self.textLayout.addText(self.text[range_end..self.len], default_opts);
                    self.textLayout.addTextDone(default_opts);
                ...
                @memset(best_spec, 0);
                @memset(best_span_len, std.math.maxInt(u16));
                @memset(best_cap, std.math.maxInt(u32));

                for (spans.items) |span| {
                    const apply_start = @max(span.start, range_start);
                    const apply_end = @min(span.end, range_end);
                    const span_len: u16 = @intCast(@min(apply_end - apply_start, std.math.maxInt(u16)));
                    var b = apply_start;
                    while (b < apply_end) : (b += 1) {
                        const i = b - range_start;
                        if (span.specificity > best_spec[i] or
                            (span.specificity == best_spec[i] and span_len < best_span_len[i]))
                        {
                            best_spec[i] = span.specificity;
                            best_span_len[i] = span_len;
                            best_cap[i] = span.capture_index;
                        }
                    }
                }
                ...
                var run: usize = 0;
                while (run < range_len) {
                    const spec = best_spec[run];
                    const cap_idx = best_cap[run];
                    var run_end = run + 1;
                    while (run_end < range_len and best_spec[run_end] == spec and best_cap[run_end] == cap_idx) : (run_end += 1) {}

                    const abs_start = range_start + run;
                    const abs_end = range_start + run_end;
                    var opts = default_opts;
                    if (spec > 0 and cap_idx != std.math.maxInt(u32)) {
                        var cap_len: u32 = undefined;
                        const cap_name = dvui.c.ts_query_capture_name_for_id(ts_parser.query, cap_idx, &cap_len);
                        opts = SyntaxHighlight.optsForCapture(cap_name[0..cap_len], ts.highlights).opts;
                    }
                    self.textLayout.addText(self.text[abs_start..abs_end], opts);
                    run = run_end;
                }
            }

            if (range_end < self.len) {
                self.textLayout.addText(self.text[range_end..self.len], default_opts);
            }
...
pub fn drawAfterText(self: *TextEntryWidget) void {
    ...
    if (focused and self.init_opts.show_focus_border) {
